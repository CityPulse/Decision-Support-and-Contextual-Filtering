...
class Route(object):
	"""docstring for Route"""
	def __init__(self, id, points, distance, time):
...
def initRoutes(starting_point, ending_point, cost_mode, number_of_routes):
	...
	#	ws = create_connection("ws://localhost:7686")
		...
		for x in xrange(numberOfRoutes):
			...
			result =  ws.recv()
			print "Received '%s'" % result
			p = re.compile('\w+;\w+;\w+;\w+;\w+;\w+\n(LINESTRING|MULTILINESTRING)\((.+)\);(\d+);(\d+);(\d+.\d+);(\d+);(\d+)')
			...
			if m :
				...
				if m.group(1) == u'LINESTRING' :
					# print(m.group(1))
					route = Route(int(m.group(6)), m.group(2).encode().split(','),int(m.group(3)),int(m.group(4)))
