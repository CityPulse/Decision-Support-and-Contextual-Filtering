...
/**
 * This class implements the Contextual Filter to identify the critical events
...
public class ContextualEventFilter {
	 ...
	 *            - the request for the ContextualEventFilter
	 * @return
	 */
	...
	/**
	 * This method calls Clingo to peform reasoning over a list of new detected
	 * events in order to identify critical events
	 *
	 * @param cityEvents
			...
			if (!answerSetList.isEmpty()) {
				for (final AnswerSet answerSet : answerSetList) {
					...
					final CityEvent criticalEvent = getCriticalEvent(answerSet,
							cityEvents);
					if (criticalEvent.getContextualCriticality() > -1) {
...
//						this.session.getAsyncRemote().sendText(gsonStr);
	...
	public void receiveUserGPSCoordinate(Coordinate userCoor) {
		...
		Point preUserCoor = (Point) this.userContext.getActivity().getActivityPlace();
		
		if(preUserCoor.getPoint().getLongitude() == userCoor.getLongitude() && preUserCoor.getPoint().getLatitude() == userCoor.getLatitude()){
	...
	/**
	 * This method stops the ContextualEventFilter by closing the
	 * CityEventConsumer
	 */
	public void stopCFF() {
